...
module.exports = function (req, res){
  ...
  function userInfoResponse(error, response, body){
    if (error) {
      ...
      console.log(require('util').inspect(_data, { depth: null }));

      var attr = {
        provider: 'facebook',
        facebookId: _data.id,
        facebookName: _data.name,
        facebookUsername: _data.name.replace(' ', ''),
        facebookEmail: _data.email
      };
      ...
      if(req.session.authenticated){
        ...
        waterlock.engine.findOrCreateAuth({/* facebookId: attr.facebookId,  */email: attr.email}, attr, userFound);
      }
    }
  }
